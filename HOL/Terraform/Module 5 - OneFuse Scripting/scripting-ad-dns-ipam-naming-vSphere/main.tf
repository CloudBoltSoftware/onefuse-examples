...
resource "onefuse_scripting_deployment" "apache" {
    policy_id = data.onefuse_scripting_policy.policy.id
    workspace_url = var.workspace_url
    template_properties = {
        "OneFuse_VmNic0.fqdn" = onefuse_ipam_record.ipam-record.ip_address 
    }
    depends_on = [time_sleep.wait_60_seconds]
}

resource "time_sleep" "wait_60_seconds" {
  depends_on = [vsphere_virtual_machine.vsphereweb1]
